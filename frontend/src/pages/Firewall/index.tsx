...
export const Firewall = () => {
    ...
    useEffect(()=> {
        if(rules.isError)
            errorNotify("Обновление файрвола не удалось!", getErrorMessage(rules.error))
    },[rules.isError])
    ...
    const enableFirewallRaw = () => {
      ...
      .then(()=>okNotify("Файрфол включен", "Файрфол был включен"))
      .catch((e)=>errorNotify("Не удалось включить файрвол!", getErrorMessage(e)))
    }

    const disableFirewallRaw = () => {
      return firewall.disable()
      .then(()=>okNotify("Файрфол выключен", "Файрфол был выключен"))
      .catch((e)=>errorNotify("Не удалось выключить файрвол!", getErrorMessage(e)))
    }
    ...
    const applyChangesRaw = () => {
      ...
      if (parsedRules === undefined){
        errorNotify("Не удалось применить правила файрвола!", "Правила файрвола недействительны")
        return Promise.reject()
      }else{
        return firewall.ruleset({rules:state, policy:currentPolicy})
        .then(()=>okNotify("Применены правила файрвола", "Правила файрвола были применены"))
        .catch((e)=>errorNotify("Не удалось применить правила файрвола!", getErrorMessage(e)))
      }
    }
    ...
    return <>
        ...
        <Box className={isMedium?'center-flex':'center-flex-row'}>
            <Title order={3}>Правила фаервола</Title>
            ...
            <Box className='center-flex'>
              <Space w="xs" />
              <Badge size="sm" color="green" variant="filled">Правила: {rules.isLoading?0:rules.data?.rules.length}</Badge>
              <Space w="xs" />
        ...
        <Tabs variant="pills" value={selectedTab} onChange={(v)=>setSelectedTab(v==NFTables.MANGLE?NFTables.MANGLE:NFTables.FILTER)} style={{ display:"flex", justifyContent:"center", alignItems:"center"}}>
            <Box mr="md">Таблица фильтрации:</Box>
            <Tabs.List>
    ...
    <Space h="xl"/> <Title className='center-flex' style={{textAlign:"center"}} order={3}>Правило не найдено! Добавьте одно, нажав кнопку "+"</Title>
    ...
    <Box className='center-flex'>
      ...
      <YesNoModal
          title='Вы уверены, что хотите применить изменения в файрволе?'
          description={`Если есть неверная конфигурация, вы можете потерять доступ к своему серверу! ⚠️`}
