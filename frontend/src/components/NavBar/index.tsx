...
export default function NavBar() {
    ...
    return <AppShell.Navbar p="md" hidden={!navOpened}>
        <Box px="xs" mt="xs">
            <Title order={4}>Настройка</Title>
        </Box>
        <Divider my="xs" />
        <Box style={{flexGrow: 1}} component={ScrollArea} px="xs" mt="xs">
            <NavBarButton navigate="nfregex" closeNav={closeNav} name="Сетевой фильтр(regrex)" color="lime" icon={<IoMdGitNetwork />} />
            <NavBarButton navigate="firewall" closeNav={closeNav} name="Правила фаервола" color="red" icon={<PiWallLight />} />
            <NavBarButton navigate="porthijack" closeNav={closeNav} name="Перенаправление портов" color="blue" icon={<GrDirections />} />
        </Box>
